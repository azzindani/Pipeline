...
! Shape below is what `pipeline_session.handover` actually returns. Keep it in
step with `HandoverPacket` in `pipeline-memory` — a documented field the code
does not produce is a promise an agent will act on and find missing.
...
```json
{
  ...
  "last_run": {
    ...
    "stdout": "...",
    "stderr": "...",
    "failure_json": "{\"message\":\"...\",\"file\":\"tests/test_auth.py\",\"line\":42}"
  },
  "recent_failures": [
    {
      "stage": "integration",
      ...
      "line": 42,
      "fix_applied": "set JWT_SECRET in .env.test",
      "fix_worked": 1
    }
  ],
  "active_work": {
  ...
  "progress": {
    ...
    "remaining": ["set JWT_SECRET in .env.test", "rerun integration"],
    "blocker": "JWT_SECRET missing in the test environment",
    "updated_at": "..."
  }
}
```
...
| Field | Source |
|---|---|
| `project.current_branch` | git at packet build time · ✗ stored state, which goes stale on checkout |
| `project.last_good_commit` | newest run with `status = pass` and a recorded `commit_sha` |
| `last_run` · `recent_failures` | `pipeline_runs` · `failures` |
| `active_work` | the stored plan — what the project intends to build |
| `progress` | the thread — what this effort has done and what is next (`session.progress`) |

`active_work` answers "what are we building"; `progress` answers "where was I".
A context reset loses the second, ✗ the first, which is why both exist.
